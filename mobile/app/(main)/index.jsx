...
export default function JournalScreen() {
  ...
  const {addJournal, journals, deleteJournal, getJournals, setCurrentJournal, journalsLoaded, currentJournal} = useMainStore()
  ...
  const handleAddJournal = async () => {
    ...
    for (let i = 0; i < journals.length; i++) {
      allNames.push(journals[i].name.toLowerCase())
    }
    ...
    if (journalName.length > 8) {
     ...
     if (allNames.includes(journalName.toLowerCase())) {
      const titleCaseName = journalName.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
      setSupMessage(`${titleCaseName} already exists!`)
      return;
     }
  ...
  return (
    <>
      ...
      : <SplashScreenView bgcolor={COLOR.magenta1} />}
    </>
  );
}
